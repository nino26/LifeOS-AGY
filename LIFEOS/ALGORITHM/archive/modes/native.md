...
Even though no ISA is created, NATIVE response runs record session metadata to `MEMORY/STATE/work.json` for observability. The Pulse Agents page surfaces this for parity — Jonathan can see all his work in one place regardless of which layer drove it.
