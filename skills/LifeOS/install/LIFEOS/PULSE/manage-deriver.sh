...
PULSE_DIR="$HOME/.gemini/config/LIFEOS/PULSE"
PLIST_NAME="com.lifeos.deriver"
PLIST_SRC="$PULSE_DIR/$PLIST_NAME.plist"
PLIST_DST="$HOME/Library/LaunchAgents/$PLIST_NAME.plist"
OBSERVABILITY_DIR="$HOME/.gemini/config/LIFEOS/MEMORY/OBSERVABILITY"
...
case "${1:-}" in
  ...
  run-now)
    ...
    exec "$BUN_PATH" run "$HOME/.gemini/config/LIFEOS/TOOLS/LearningPatternSynthesis.ts" --hypothesize
    ;;
