...
const HOME = homedir();
const DB_PATH = join(HOME, ".gemini/config/LIFEOS/MEMORY/STATE/feed-candidates.db");
const CFENV = join(HOME, ".gemini/config/skills/_CLOUDFLARE/Tools/CfEnv.ts");
const INFERENCE = join(HOME, ".gemini/config/LIFEOS/TOOLS/Inference.ts");
...
async function cmdHarvest() {
  ...
  console.log(`Shortlist: bun ${join(HOME, ".gemini/config/LIFEOS/TOOLS/BlogDiscovery.ts")} top --n 50`);
}
