 ...
 *
 *   bun ~/Projects/LifeOS-AGY/LIFEOS/TOOLS/InstallConveyorRunner.ts             # install
 *   bun ~/Projects/LifeOS-AGY/LIFEOS/TOOLS/InstallConveyorRunner.ts --uninstall # remove
 *   bun ~/Projects/LifeOS-AGY/LIFEOS/TOOLS/InstallConveyorRunner.ts --status    # check
 *
...
const TEMPLATE_PATH = join(HOME, ".gemini/config", "LIFEOS", "TOOLS", "com.lifeos.conveyor-runner.plist.template");
...
async function linuxSpec(): Promise<systemd.UnitSpec> {
  ...
  return {
    ...
    exec: [bunPath, join(HOME, ".gemini/config", "LIFEOS", "TOOLS", "Conveyor", "Runner.ts")],
    logPath: join(HOME, ".gemini/config", "LIFEOS", "MEMORY", "STATE", "com.lifeos.conveyor-runner.log"),
