...
function mainLogs(): string[] {
  return glob(join(homedir(), ".gemini/config/projects/*/*.jsonl"));
}

function sideLogs(): string[] {
  return glob(join(homedir(), ".gemini/config/projects/*/*/subagents/*.jsonl"));
}
...
function loadLatestApiCostSnapshot(): ApiCostSnapshot | null {
  const path = join(homedir(), ".gemini/config/LIFEOS/MEMORY/OBSERVABILITY/anthropic-cost.jsonl");
  let raw: string;
  try {
    raw = readFileSync(path, "utf8");
  } catch {
