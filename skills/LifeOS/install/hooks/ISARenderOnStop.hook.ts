...
const STATE_DIR = join(homedir(), '.gemini/config/LIFEOS/MEMORY/STATE/isa-render-debounce');
const ISA_RENDER = join(homedir(), '.gemini/config/LIFEOS/TOOLS/ISARender.ts');
...
if (rendered.length || skipped.length) {
  try {
    ...
    const logDir = join(homedir(), '.gemini/config/LIFEOS/MEMORY/OBSERVABILITY');
