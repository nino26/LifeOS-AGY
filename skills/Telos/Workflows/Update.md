...
TELOS is {PRINCIPAL.NAME}'s life framework stored in `~/Projects/LifeOS-AGY/LIFEOS/USER/TELOS/`. The list below is the full file taxonomy — a fresh install's starter scaffold ships a subset, files are created as life data accumulates, and the workflow skips any file not yet present. It contains:
...
!`FILE="$1"; CONTENT="$2"; DESCRIPTION="$3"; bun ~/Projects/LifeOS-AGY/.agents/skills/Telos/Tools/UpdateTelos.ts "$FILE" "$CONTENT" "$DESCRIPTION"`
...
!`head -50 ~/Projects/LifeOS-AGY/LIFEOS/USER/TELOS/updates.md`

## View Specific TELOS File
!`FILE="$1"; cat ~/Projects/LifeOS-AGY/LIFEOS/USER/TELOS/"$FILE"`
...
Example:
```bash
bun ~/Projects/LifeOS-AGY/.agents/skills/Telos/Tools/UpdateTelos.ts "BOOKS.md" "- *Project Hail Mary* by Andy Weir" "Added favorite book: Project Hail Mary"
```
...
The script is at: `~/Projects/LifeOS-AGY/.agents/skills/Telos/Tools/UpdateTelos.ts`

All backups are stored in: `~/Projects/LifeOS-AGY/LIFEOS/USER/TELOS/Backups/`

All changes are logged in the `## Changelog` section at the bottom of `~/Projects/LifeOS-AGY/LIFEOS/USER/TELOS/TELOS.md`.
