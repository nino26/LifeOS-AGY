...
- World models must exist at `~/Projects/LifeOS-AGY/MEMORY/RESEARCH/WorldModels/`
...
```
Check ~/Projects/LifeOS-AGY/MEMORY/RESEARCH/WorldModels/ for all 11 model files.
...
curl -s -X POST http://localhost:8888/notify \
  ...
  -d '{"message": "Testing your idea against all eleven world threat models at TIER tier", "voice_id": "21m00Tcm4TlvDq8ikWAM"}'
```
...
Read all 11 model files from `~/Projects/LifeOS-AGY/MEMORY/RESEARCH/WorldModels/`.
...
curl -s -X POST http://localhost:8888/notify \
  -H "Content-Type: application/json" \
  -d '{"message": "Analysis complete. SUMMARY_OF_EXECUTIVE_VERDICT", "voice_id": "21m00Tcm4TlvDq8ikWAM"}'
```
