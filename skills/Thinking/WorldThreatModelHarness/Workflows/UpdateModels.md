...
- Model template at `~/Projects/LifeOS-AGY/.agents/skills/WorldThreatModelHarness/ModelTemplate.md`
...
```
Read ~/Projects/LifeOS-AGY/MEMORY/RESEARCH/WorldModels/INDEX.md (if exists)
...
curl -s -X POST http://localhost:8888/notify \
  ...
  -d '{"message": "Updating world threat models. This will take several minutes as I research current state for each time horizon.", "voice_id": "21m00Tcm4TlvDq8ikWAM"}'
```
...
Save to: `~/Projects/LifeOS-AGY/MEMORY/RESEARCH/WorldModels/{horizon}.md`

### Step 5: Update INDEX

Write/update `~/Projects/LifeOS-AGY/MEMORY/RESEARCH/WorldModels/INDEX.md`:
...
curl -s -X POST http://localhost:8888/notify \
  -H "Content-Type: application/json" \
  -d '{"message": "World models updated. N horizons refreshed with current research.", "voice_id": "21m00Tcm4TlvDq8ikWAM"}'
```
