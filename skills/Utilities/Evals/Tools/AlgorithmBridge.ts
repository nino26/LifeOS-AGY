...
export async function updateISCWithResult(result: AlgorithmEvalResult): Promise<void> {
  ...
  await $`bun run ~/Projects/LifeOS-AGY/.agents/skills/THEALGORITHM/Tools/ISCManager.ts update --row ${result.isc_row} --status ${status} --note "${formatForISC(result)}"`.quiet();
}
