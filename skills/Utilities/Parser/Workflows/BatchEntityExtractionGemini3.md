...
const entityIndex = JSON.parse(
  fs.readFileSync('~/Projects/LifeOS-AGY/.agents/skills/parser/entity-index.json', 'utf-8')
);
...
```typescript
import { scrapeAsMarkdown } from '~/Projects/LifeOS-AGY/.agents/skills/mcp/Providers/brightdata/actors';
...
// Main processing
const entityIndexPath = '~/Projects/LifeOS-AGY/.agents/skills/parser/entity-index.json';
...
const schema = JSON.parse(
  fs.readFileSync('~/Projects/LifeOS-AGY/.agents/skills/parser/schema/content-schema.json', 'utf-8')
);
...
processedArticles.forEach((article: any, index: number) => {
  ...
  if (!valid) {
    ...
    const filename = `~/Projects/LifeOS-AGY/.agents/skills/parser/output/${timestamp}_batch-${index + 1}.json`;
...
```bash
# Check entity counts
cat ~/Projects/LifeOS-AGY/.agents/skills/parser/entity-index.json | jq '{
