...
| **Antigravity CLI handoff bundle** | Production code pipeline — structured for `frontend-design` plugin |
...
> "Claude packages everything into a handoff bundle that you can pass to Antigravity CLI with a single instruction."

This is the load-bearing integration point between Claude Design (concept/design) and Antigravity CLI (production). The `frontend-design` plugin (installed via Anthropic's official plugins marketplace) auto-activates when the bundle lands in Antigravity CLI.
...
As of the June 2026 update, Claude Design usage shares one pool with claude.ai chat, Antigravity CLI, and Cowork — it is no longer a separate quota.
...
- **`/design-sync` (Antigravity CLI)** — bidirectional sync: pull codebase design system into Claude Design, or push built output back.
- **`/design` (Antigravity CLI)** — create/edit/sync designs from the Antigravity CLI terminal.
- **Expanded export destinations** — Adobe, Base44, Canva, Gamma, Lovable, Miro, Replit, Vercel, Wix, plus PDF/PPTX.
- **Shared token pool** — usage now shares limits with chat, Antigravity CLI, and Cowork.
...
| Production code | **`frontend-design` plugin** | Antigravity CLI (auto-activates) | Turns handoff bundles into production-grade code |
