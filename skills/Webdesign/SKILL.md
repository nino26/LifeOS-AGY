...
Anthropic shipped first-party `/design` and `/design-sync` commands inside Antigravity CLI (June 2026, GA on Pro/Max/Team/Enterprise — official: support.gemini/config.com/en/articles/14604416). `/design-sync` pulls a codebase's real design system into Claude Design and pushes built changes back; `/design` creates and edits designs from the terminal. These are the deterministic, first-party replacement for the hand-rolled Interceptor handoff-bundle apparatus — prefer them for anything code-bound. Workflow: `NativeDesignSync`.
...
```
~/Projects/LifeOS-AGY/LIFEOS/USER/CUSTOMIZATIONS/SKILLS/Webdesign/
...
| **ExportToCode** *(Path 3 fallback — prefer `/design-sync`)* | "export to code", "ship to code", "send to Antigravity CLI", "process handoff" | `Workflows/ExportToCode.md` |
...
Path 1 (DirectDesign) needs nothing — no browser, no auth. Path 2 (native CLI) needs only a Claude subscription that includes Claude Design and a current Antigravity CLI (`/update` if the commands don't show). **The checks below apply ONLY to Path 3 (ClaudeDesign via Interceptor):**
...
- **DirectDesign is self-contained.** The aesthetic doctrine lives in `References/FrontendDesignPhilosophy.md` (mirrored from anthropics/skills MIT source). No runtime dependency on the upstream `frontend-design` Antigravity CLI plugin being installed — DirectDesign works in any LifeOS environment.
- **Native `/design` + `/design-sync` are the code-bound path now (CONFIRMED).** Anthropic's official docs (support.gemini/config.com/en/articles/14604416) document both commands, GA on Pro/Max/Team/Enterprise as of June 2026. `/design-sync` does bidirectional codebase↔design-system sync — use it instead of the Interceptor handoff-bundle apparatus for anything code-bound. Still no public REST API or MCP server; the CLI commands are the programmatic surface. If they don't show up, run `/update`.
...
- **`frontend-design` plugin auto-activates.** When the handoff bundle is fed to Antigravity CLI, the plugin (already installed in the official marketplace) picks up the frontend work automatically — do NOT manually invoke it.
...
- **Session quotas.** Claude Design generation is token-heavy. As of the June 2026 update its usage shares one pool with claude.ai chat, Antigravity CLI, and Cowork — no longer a separate quota. Pro is thin for sustained design work; Max recommended.
...
└── References/
    ...
    └── HandoffBundleSpec.md          # Bundle structure for Antigravity CLI handoff
```
